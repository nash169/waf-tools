    ...
    def match_libs(lib_names, is_static):
        ...
        for lib in lib_names:
            if lib == "python":
                ...
                tags = "({0})?((-py{2})|(-py{1}(?=[^0-9]))|({2})|({1}(?=[^0-9]))|(?=[^0-9])(?!-py))".format(
                    tags_pat, kw["python"][0], kw["python"]
                )
            else:
                tags = tags_pat
...
def check_boost(self, *k, **kw):
    ...
    params = {"lib": k and k[0] or kw.get("lib", None), "stlib": kw.get("stlib", None)}
    ...
    def try_link():
        if (
            (params["lib"] and "system" in params["lib"])
            or params["stlib"]
            and "system" in params["stlib"]
        ):
            self.check_cxx(fragment=BOOST_ERROR_CODE, use=var, execute=False)
        if (
            (params["lib"] and "thread" in params["lib"])
            or params["stlib"]
            and "thread" in params["stlib"]
        ):
